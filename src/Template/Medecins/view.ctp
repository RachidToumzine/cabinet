<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Medecin $medecin
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Medecin'), ['action' => 'edit', $medecin->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Medecin'), ['action' => 'delete', $medecin->id], ['confirm' => __('Are you sure you want to delete # {0}?', $medecin->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Medecins'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Medecin'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Users'), ['controller' => 'Users', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New User'), ['controller' => 'Users', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="medecins view large-9 medium-8 columns content">
    <h3><?= h($medecin->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Nom') ?></th>
            <td><?= h($medecin->nom) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Prenom') ?></th>
            <td><?= h($medecin->prenom) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Photo') ?></th>
            <td><?= h($medecin->photo) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('User') ?></th>
            <td><?= $medecin->has('user') ? $this->Html->link($medecin->user->id, ['controller' => 'Users', 'action' => 'view', $medecin->user->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($medecin->id) ?></td>
        </tr>
    </table>
</div>
